...
local theme = {
	...
	bg_color = beautiful.bg_normal .. "CC",
	box_margin = dpi(40),
}
		...
		end,
		{
			...
			margins = theme.box_margin,
			{
				...
				},
				{
					...
					{
						widget = wibox.widget.textbox,
						markup = "<b>Tag:</b><i> " .. generate_string_of_numbers(0, 9) .. "</i>",
						font = beautiful.font .. " 20",
					},
					{
						widget = wibox.widget.textbox,
...
M.setup = function(logic)
	logic.register_keygrabber_stop_callback(function(user_input)
		if user_input.screen_index ~= nil and user_input.tag_index ~= nil then
			local s = screen[user_input.screen_index]
			local c = client.focus
			if c ~= nil then
				c:move_to_screen(s)
				c:move_to_tag(s.tags[user_input.tag_index])
			end
		end
...
M.start = function(logic)
	...
	for entry in screen do
		local screen_geometry = entry.geometry
		...
		if entry.index == focused_screen.index then
			...
			attached_popups[entry.index].set_content({ focused_screen_content, help_box })
		else
